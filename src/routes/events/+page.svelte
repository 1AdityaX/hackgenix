...
<div class="flex h-screen flex-col w-full">
	<header class="w-full border-b border-zinc-200 bg-white px-4 py-4 shadow-sm dark:border-zinc-700 dark:bg-zinc-800 sm:px-6">
		...
		<div class="mx-auto h-full w-full max-w-[calc(100%-16rem)] overflow-y-auto p-6">
			<div class="space-y-8">
				{#each sortedEvents as event}
					<article class="overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-sm transition-all hover:shadow-md dark:border-zinc-700 dark:bg-zinc-800">
								...
								<div class="flex-1">
									...
									<div class="mt-4 space-y-3 text-sm text-zinc-600 dark:text-zinc-300">
										<div class="flex items-center">
											<svg class="mr-3 h-4 w-4 shrink-0 text-zinc-500 dark:text-zinc-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
												<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
											</svg>
											{formatDate(event.date)} • {formatTime(event.start_time)} - {formatTime(event.end_time)}
										</div>
										<div class="flex items-start">
											<svg class="mr-3 mt-0.5 h-4 w-4 shrink-0 text-zinc-500 dark:text-zinc-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
